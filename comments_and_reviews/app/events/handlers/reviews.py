...
async def add_automoderation_request(services: ServicesFactory, complainant: Actor, review_id: str):
    moderation = services.get_reviews_moderation_service()
    await moderation.create_request(complainant, review_id, "Automoderation")


@reviews_router.add_protected(EventType.CREATE_REVIEW, CreateReview)
async def handle_create_review(token: str, body: CreateReview, services: ServicesFactory) -> Review:
...
@reviews_router.add_protected(EventType.UPDATE_REVIEW, UpdateReview)
async def handle_update_review(token: str, body: UpdateReview, services: ServicesFactory) -> Review:
    actor = get_actor(token)
    reviews = services.get_reviews_service()
...
@reviews_router.add_protected(EventType.DELETE_REVIEW, Id)
async def handle_delete_review(token: str, body: Id, services: ServicesFactory) -> Review:
    actor = get_actor(token)
    reviews = services.get_reviews_service()
...
@reviews_router.add(EventType.GET_REVIEWS_BY_PRODUCT, Id)
async def handle_get_reviews_by_product_id(body: Id, services: ServicesFactory) -> ReadReviews:
    reviews = services.get_reviews_service()
    res = await reviews.get_reviews_by_product(body.value)
    return ReadReviews(reviews=res)


@reviews_router.add(EventType.GET_REVIEWS_BY_USER, Id)
async def handle_get_reviews_by_user(body: Id, services: ServicesFactory) -> ReadReviews:
    reviews = services.get_reviews_service()
    res = await reviews.get_reviews_by_user(body.value)
    return ReadReviews(reviews=res)


@reviews_router.add(EventType.GET_REVIEW_BY_ID, Id)
async def handle_get_review_by_id(body: Id, services: ServicesFactory) -> ReadReviews:
    reviews = services.get_reviews_service()
...
@reviews_router.add(EventType.GET_PRODUCT_STATS, Id)
async def handle_get_product_stats(body: Id, services: ServicesFactory) -> ReviewsStatistics:
    stats = services.get_reviews_statistics_service()
